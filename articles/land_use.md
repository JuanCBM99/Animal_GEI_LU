...
- **`grassland_unconvertible`:** Permanent native pastures, upland
  ...
  marginal soils unsuitable for crop cultivation.
...
(`none`), the yield is strictly zeroed to avoid false database warnings.

Because FAOSTAT productivities (`fao_crops.parquet` and
`fao_forages.parquet`) report crop yields on an as-harvested fresh
weight basis ($`RawYield_i`$), `herdr` converts them to a dry matter
yield ($`Yield_i`$) using the ingredient’s dry matter fraction from
`user_data/feed_characteristics.csv`:
...
If a user provides a `custom_yield_kg_ha` in `diet_ingredients.csv`, it
is assumed to already represent dry matter and is used directly without
$`DM\_pct`$ scaling.
...
or milk replacer the ingredient belongs to. Within any given cohort and
category, `herdr` validates that $`\sum Share_i = 100\% \pm 0.1\%`$.
...
2.  **User custom yields:** If `custom_yield_kg_ha` is provided in
    ...
    `"Custom Data"`) and bypasses FAO databases.
3.  **FAOSTAT crop yields:** Otherwise, crop productivities are queried
    from `fao_crops.parquet` for the resolved country and reporting year
    (filtering out regional aggregate areas where `Area Code >= 5000`).
4.  **Curated forage database:** For forages and pasture grasses not
    covered in FAOSTAT arable crops, yields are queried from
    `fao_forages.parquet`.
5.  **Global mix fallback:** If a crop has no recorded yield in the
    resolved country for that reference year, `herdr` computes a
